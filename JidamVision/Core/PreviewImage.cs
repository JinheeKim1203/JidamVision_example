...
namespace JidamVision.Core
{
    ...
    public class PreviewImage
    {
        ...
        //필터 효과 기능
        public void ApplyFilter(String cmbSelectFilter1, int cmbSelectFilter2)
        {
            if (_orinalImage == null)
                return;
            Mat filteredImage = new Mat();
            switch (cmbSelectFilter1)
            {
                case "연산":
                    // 연산 관련 enum 값 매핑
                    ImageOperation operation = (ImageOperation)cmbSelectFilter2;
                    string op_values = "30 30 30";  //연산값 지정
                    ApplyImageOperation(operation, _orinalImage, op_values, out filteredImage);
                    break;
                case "비트연산":
                    // 비트 연산 관련 enum 값 매핑
                    Bitwise bitwise = (Bitwise)cmbSelectFilter2;
                    ApplyBitwiseOperation(bitwise, _orinalImage, out filteredImage);
                    break;
                case "블러링":
                    ImageFilter filter = (ImageFilter)cmbSelectFilter2;
                    ApplyImageFiltering(filter, _orinalImage, out filteredImage);
                    break;
                case "엣지":
                    ImageEdge edge = (ImageEdge)cmbSelectFilter2;
